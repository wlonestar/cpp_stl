#ifndef CPP_STL_TESTFRAME_H
#define CPP_STL_TESTFRAME_H
...
requires(std::is_base_of_v<BaseClass, DerivedClass> &&...) &&
  std::is_base_of_v<Object, BaseClass> &&requires {
  typename BaseClass::Output;
}
class TestFramework {
...
public:
  ...
  requires std::is_same_v<Output, void> ||
    std::is_invocable_r_v<Output, BaseClass, InputType...>
  void printResult(InputType... inputs) {
    ...
    for (const auto &instance: m_instances) {
      ...
      printf("TEST [%s:%zu] ", name.c_str(), width);
      try {
        if constexpr (std::is_same_v<Output, void> ||
                      std::is_reference_v<Output>) {
          auto time{reportProcedureTime(
            [&]() { (*instance)(std::forward<InputType>(inputs)...); })};
          // std::cout << std::format("Time: {0:.6f} s", time) << std::endl;
          printf("Time: %.6f s\n", time);
        } else {
          auto [result, time]{reportTime<Output>([&]() {
            return (*instance)(std::forward<InputType>(inputs)...);
          })};
          // std::cout << std::format("Result: {0}, Time: {1:.6f} s", result,
          // time) << std::endl;
          std::cout << "Result: " << result << ", Time: ";
          printf("%.6f s\n", time);
        }
...
private:
  ...
  BaseClass &operator[](size_t index) { return *m_instances[index]; }
